...
void TensorTemplate<CPU, DENSE, Dtype>::Serialize(FILE* fid)
{
	...
	fwrite(&(data->mem_size), sizeof(size_t), 1, fid);
	fwrite(data->ptr, sizeof(Dtype), data->mem_size, fid);
...
void TensorTemplate<CPU, DENSE, Dtype>::Deserialize(FILE* fid)
{
	...
	size_t new_mem_size;
	fread(&(new_mem_size), sizeof(size_t), 1, fid);
	//assert(fread(&(new_mem_size), sizeof(size_t), 1, fid) == 1);
	this->data->Resize(new_mem_size);
	fread(data->ptr, sizeof(Dtype), new_mem_size, fid);
...
{
	/**std::cout << "Doing MM [CPU DTensor], DTensor * DTensor" << std::endl;
	std::cout << "Rank of Matrix 1: " << a.rank() << std::endl;
	std::cout << "Rank of Matrix 2: " << b.rank() << std::endl;
	std::cout << "Shape of Matrix 1: row=" << a.rows() << ", col=" << a.cols() << ", TransA=";
	if (transA == Trans::N)
		std::cout << "false" << std::endl;
	else
		std::cout << "true" << std::endl;
	std::cout << "Shape of Matrix 2: row=" << b.rows() << ", col=" << b.cols() << ", TransB=";
	if (transB == Trans::N)
		std::cout << "false" << std::endl;
	else
		std::cout << "true" << std::endl;*/
...
{
	/*std::cout << "Doing MM, SpTensor * DTensor..." << std::endl;
	std::cout << "Rank of Matrix 1: " << a.rank() << std::endl;
	std::cout << "Rank of Matrix 2: " << b.rank() << std::endl;
	std::cout << "Shape of Matrix 1: row=" << a.rows() << ", col=" << a.cols() << std::endl;
	std::cout << "Shape of Matrix 2: row=" << b.rows() << ", col=" << b.cols() << std::endl;*/
...
void TensorTemplate<CPU, DENSE, int>::Serialize(FILE* fid)
{
	Tensor::Serialize(fid);
	fwrite(&(data->mem_size), sizeof(size_t), 1, fid);
	fwrite(data->ptr, sizeof(int), data->mem_size, fid);
	//assert(fwrite(&(data->mem_size), sizeof(size_t), 1, fid) == 1);
...
void TensorTemplate<CPU, DENSE, int>::Deserialize(FILE* fid)
{
	...
	//assert(fread(&(new_mem_size), sizeof(size_t), 1, fid) == 1);
	this->data->Resize(new_mem_size);
	fread(data->ptr, sizeof(int), new_mem_size, fid);
