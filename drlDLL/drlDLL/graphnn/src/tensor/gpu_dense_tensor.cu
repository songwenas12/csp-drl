...
void TensorTemplate<GPU, DENSE, Dtype>::Serialize(FILE* fid)
{
	...
	fwrite(buf, sizeof(Dtype), data->mem_size, fid);
...
void TensorTemplate<GPU, DENSE, Dtype>::Deserialize(FILE* fid)
{
	...
	fread(buf, sizeof(Dtype), new_mem_size, fid);
...
{
	//std::cout << "Doing MM [GPU Dense double], DTensor * DTensor" << std::endl;
	...
	Reshape({m, n});
	//<TODO>: check why the below code looks like B*A
	WITH_GPUCTX(ctx, {
...
{
	/*std::cout << "Doing MM [GPU Dense float], DTensor * DTensor" << std::endl;
	std::cout << "Rank of Matrix 1: " << a.rank() << std::endl;
	std::cout << "Rank of Matrix 2: " << b.rank() << std::endl;
	std::cout << "Shape of Matrix 1: row=" << a.rows() << ", col=" << a.cols() << ", TransA=";
	if (transA == Trans::N)
