...
{
	//std::cout << "Setting ref" << std::endl;
	graph = static_cast< GraphStruct* >(p);
}

//================ hypergraph variable =====================

HyperGraphVar::HyperGraphVar(std::string _name) : Variable(_name), graph(nullptr)
{

}

EleType HyperGraphVar::GetEleType()
{
	return EleType::UNKNOWN;
}

MatMode HyperGraphVar::GetMode()
{
	return MatMode::cpu;
}

void HyperGraphVar::SetRef(void* p)
{
	//std::cout << "Setting ref for HyperGraphVar" << std::endl;
	graph = static_cast<HyperGraphStruct*>(p);
}
