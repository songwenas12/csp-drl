									...
									Phase phase)
{
	...
	while (!q.empty())
	{
		...
		/*std::cout << "Forwarding factor " << factor->name << std::endl;
		for (int tmpI = 0; tmpI < operands.size(); tmpI++) {
			std::cout << "Var " << tmpI << " name=" << operands[tmpI]->name<<" ";
		}
		std::cout<<std::endl;*/

		factor->Forward(operands, outputs, phase);

		//std::cout << "Factor forward done." << std::endl;
