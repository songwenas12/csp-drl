...
namespace gnn
{
	...
	/**
	 * @brief      represents an undirected Hypergraph
	 */
	class HyperGraphStruct {
	public:
		/**
		 * @brief      construct an empty graph
		 */
		...
		/**
		 * @brief      Adds a hyperedge.
		 *
		 * @param[in]  subg_id  The subgraph identifier (used for mini-batch)
		 * @param[in]  e_idx   The edge index, must add in the increasing order of idx (start from 0)
		 * @param[in]  edgeNodeList     The nodes related in this hyperedge
		 */
		void AddEdge(int subg_id, int e_idx, std::vector<int>& edgeNodeList);
		...
		/**
		 * adjacent list representation; stores the (id of) linked edges of a node
		 * <TODO>: check if this is correct
		 */
		LinkedTable<int> *linked_edges;

		/**
		 * Stores the neighbor relations between nodes
		 */
		...
		/**
		* track the edge lists of each subgraph
		*/
		LinkedTable< int >* subgraph_edge;
