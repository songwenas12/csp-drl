...
// ----- Choose minimum q value --

int64 ChooseMinQ(Solver* solver, const std::vector<IntVar*>& vars,
	int64 first_unbound, int64 last_unbound) {
	auto t0 = std::chrono::high_resolution_clock::now();
	//Use the eps-greedy policy to choose the next variable
	bool ifGreedy = true;
	switch (solver->drl_phase) {
	case Solver::DRLPhase::WARM_UP:
		ifGreedy = false;
	case Solver::DRLPhase::TRAINING:
		ifGreedy = solver->GreedyOrNot(solver->masterDRLObj->crtEps);
	case Solver::DRLPhase::TESTING:
		ifGreedy = true;
	default:
		break;
	}
	...
	if (ifGreedy) {
		//Given the current state, feed forward the QNet
		std::vector< std::shared_ptr<ConstrStruct> > glist;
		glist.push_back(solver->instanceConstrStruct);
		std::vector< std::shared_ptr<ConstrFeatures> > stateFeature;
		stateFeature.push_back(std::make_shared<ConstrFeatures>(solver, false));
		solver->masterDRLObj->Predict(glist, stateFeature, solver->masterDRLObj->predVectors);
	}
	//Get the ID of all unbounded vars
	std::vector<int64> unbounded;
	...
	//Choose the best var
	double best_Q = solver->masterDRLObj->predVectors[0]->data()[unbounded[0]];
	int64 best_index = unbounded[0];
	if (ifGreedy) {
		for (int64 varID : unbounded) {
			double pred_Q = solver->masterDRLObj->predVectors[0]->data()[varID];
			if (pred_Q < best_Q) {
				best_Q = pred_Q;
				best_index = varID;
			}
		...
		auto time = t1 - t0;
		auto durationT = std::chrono::duration_cast<std::chrono::microseconds>(time).count();
		solver->inferenceTime += durationT;
		solver->inferenceCount++;
		//Update running estimate
		solver->avgInferTime = solver->avgInferTime + (durationT - solver->avgInferTime) / solver->inferenceCount;
	}
	else {
		int idx = rand() % unbounded.size();
		best_index = unbounded[idx];
	}

	if (solver->drl_phase == Solver::DRLPhase::WARM_UP) {
		//Use ChooseMinSizeOverTdeg
		// For each constraint, compute the tightness
		std::vector<double> tightness;
		for (int constrID = 0; constrID < solver->constrList.size(); constrID++) {
			//Compute the product of domain sizes
			int domainProduct = 1;
			for (int j = 0; j < solver->instanceConstrStruct->constrVarIndex[constrID].size(); j++) {
				domainProduct *= solver->varList[solver->instanceConstrStruct->
					constrVarIndex[constrID][j]]->Size();
			}
			//Compute the tightness
			uint64 numActiveTuples = solver->constrList[constrID]->numActiveTuples();
			tightness.push_back(1 - (double)numActiveTuples / domainProduct);
		}
		// For each constraint, compute the number of unbounded variables in its scope
		std::vector<int> num_unbounded;
		for (int j = 0; j < solver->constrList.size(); j++) {
			int tmpCount = 0;
			for (int i : solver->instanceConstrStruct->constrVarIndex[j]) {
				if (vars[i]->Bound() == false)
					tmpCount++;
			}
			num_unbounded.push_back(tmpCount);
		}
		// For each variable, compute the dynamic tightness degree
		std::vector<double> Tdeg;
		for (int i = 0; i < solver->varList.size(); i++) {
			double tmpSum = 0.0;
			for (int j : solver->instanceConstrStruct->varConstrIndex[i]) {
				if (num_unbounded[j] > 1)
					tmpSum += tightness[j];
			}
			Tdeg.push_back(tmpSum);
		}
		// Find each unbounded variable, compute Dom/Tdeg
		std::vector<double> dom_over_Tdeg(solver->varList.size(), 0.0);
		for (int i = 0; i < solver->varList.size(); i++) {
			if (vars[i]->Bound() == false) {
				dom_over_Tdeg[i] = vars[i]->Size() / Tdeg[i];
			}
		}
		//Return the unbounded variable with the minimum Dom/Ddeg
		double best_value = dom_over_Tdeg[first_unbound];
		int64 best_index = first_unbound;
		for (int64 i = first_unbound; i <= last_unbound; ++i) {
			IntVar* const var = vars[i];
			if (!var->Bound()) {
				if (dom_over_Tdeg[i] < best_value) {
					best_value = dom_over_Tdeg[i];
					best_index = i;
				}
			}
		}
...
int64 ChooseMinQ_Test(Solver* solver, const std::vector<IntVar*>& vars,
	int64 first_unbound, int64 last_unbound) {
	auto t0 = std::chrono::high_resolution_clock::now();

	if (solver->masterDRLObj->maxInferDepth > 0 && solver->SearchDepth() > solver->masterDRLObj->maxInferDepth) {
		//std::cout << "Reaching maximum inference level" << std::endl;
		// Using Dom/Ddeg
		// For each constraint, compute the number of unbounded variables in its scope
		std::vector<int> num_unbounded;
		for (int j = 0; j < solver->constrList.size(); j++) {
		...
		// For each variable, compute the dynamic degree
		std::vector<int> Ddeg;
		for (int i = 0; i < solver->varList.size(); i++) {
			int tmpCount = 0;
			for (int j : solver->instanceConstrStruct->varConstrIndex[i]) {
			...
			Ddeg.push_back(tmpCount);
		}
		// Find each unbounded variable, compute Dom/Ddeg
		std::vector<double> dom_over_Ddeg(solver->varList.size(), 0.0);
		for (int i = 0; i < solver->varList.size(); i++) {
			if (vars[i]->Bound() == false) {
				dom_over_Ddeg[i] = (double)(vars[i]->Size()) / Ddeg[i];
			}
		}
		//Return the unbounded variable with the minimum Dom/Ddeg
		double best_value = dom_over_Ddeg[first_unbound];
		int64 best_index = first_unbound;
		for (int64 i = first_unbound; i <= last_unbound; ++i) {
			IntVar* const var = vars[i];
			if (!var->Bound()) {
				if (dom_over_Ddeg[i] < best_value) {
					best_value = dom_over_Ddeg[i];
					best_index = i;
				}
			}
		}
	...
	//Use the greedy policy to choose the next variable
	
	//Given the current state, feed forward the QNet
	std::vector< std::shared_ptr<ConstrStruct> > glist;
	glist.push_back(solver->instanceConstrStruct);
	std::vector< std::shared_ptr<ConstrFeatures> > stateFeature;
	stateFeature.push_back(std::make_shared<ConstrFeatures>(solver, /*solver->masterDRLObj->numConstrCheckSample,*/ false));

	solver->masterDRLObj->Predict(glist, stateFeature, solver->masterDRLObj->predVectors);

	//Choose the best var
	double best_Q = solver->masterDRLObj->predVectors[0]->data()[first_unbound];
	double pred_Q = 0.0;
	int64 best_index = first_unbound;
	for (int i = first_unbound; i <= last_unbound; i++) {
		IntVar* const var = vars[i];
		if (!var->Bound()) {
			pred_Q = solver->masterDRLObj->predVectors[0]->data()[i];
			if (pred_Q < best_Q) {
				best_Q = pred_Q;
				best_index = i;
			}	
	...
	auto t1 = std::chrono::high_resolution_clock::now();
	auto time = t1 - t0;
	auto dur_Infer = std::chrono::duration_cast<std::chrono::microseconds>(time).count();
	solver->inferenceTime += dur_Infer;
	solver->inferenceCount++;
...
int64 ChooseMinSizeOverDdeg(Solver* solver, const std::vector<IntVar*>& vars,
	int64 first_unbound, int64 last_unbound) {
	// For each constraint, compute the number of unbounded variables in its scope
	std::vector<int> num_unbounded;
	for (int j = 0; j < solver->constrList.size(); j++) {
...
int64 ChooseMinSizeOverTdeg(Solver* solver, const std::vector<IntVar*>& vars,
	int64 first_unbound, int64 last_unbound) {
	// For each constraint, compute the tightness
	std::vector<double> tightness;
	for (int constrID = 0; constrID < solver->constrList.size(); constrID++) {
...
class VariableAssignmentSelector : public BaseVariableAssignmentSelector {
 public:
  ...
  int64 ChooseVariable() override {
	 // std::cout<<"Choosing Var, selector type is "<<typeid(var_selector_).name()<<std::endl;
...
class AssignOneVariableValue : public Decision {
 public:
  ...
  int getVarID(std::vector<IntVar*> varList) {
	  for (int i = 0; i < varList.size(); i++) {
		  if (varList[i] == this->var_) {
			  return i;
		  }
	  }
	  return -1;
...
void AssignOneVariableValue::Apply(Solver* const s) { 
	//std::cout<<"Apply-AssignOneVariableValue"<<std::endl;
	//std::cout<<"Type of var: "<<typeid(*var_).name()<<std::endl;
	//std::cout<<"Before  "<<var_<<std::endl;
	var_->SetValue(value_);
	//std::cout<<"After  "<<var_<<std::endl;
}

void AssignOneVariableValue::Refute(Solver* const s) {
	//std::cout<<"Refute-AssignOneVariableValue"<<std::endl;
  var_->RemoveValue(value_);	// This is the place for Propagation. If unsuccessful, the function will
								// not continue, i.e. the below info will not be output.
  //std::cout << "RemoveValue done" << std::endl;
}
}  // namespace
 ...
 public:
  ...
  static Solver::VariableIndexSelector MakeVariableSelector(
    ...
    switch (str) {
	  ...
	  case Solver::CHOOSE_MIN_Q:
		  return ChooseMinQ;
	  case Solver::CHOOSE_MIN_Q_TEST:
		  return ChooseMinQ_Test;
	  case Solver::CHOOSE_MIN_SIZE_OVER_DDEG:
		  return ChooseMinSizeOverDdeg;
	  case Solver::CHOOSE_MIN_SIZE_OVER_TDEG:
		  return ChooseMinSizeOverTdeg;
      default:
  ...
  //ADDED: return the selector so that the vars_ can be retrieved
  BaseVariableAssignmentSelector* getSelector() {
...
Decision* BaseAssignVariables::Next(Solver* const s) {
  ...
  //std::cout<<"Selector type: "<<typeid(*selector_).name()<<std::endl;
  /*std::cout<<"Vectors["<<vars.size()<<"]: ";
  ...
  if (id >= 0 && id < vars.size()) {
	...
	// std::cout << "ChosenVar: " << var << ", chosenValue=" << value <<std::endl;
    switch (mode_) {
      case ASSIGN:
...
OptimizeVar::OptimizeVar(Solver* const s, bool maximize, IntVar* const a,
    ...
    : SearchMonitor(s),
      ...
      found_initial_solution_(false) {
		 // std::cout<<"Construct optimizerVar, type of var_ is"<<typeid(*a).name()<<std::endl;
...
void OptimizeVar::EnterSearch() {
	//std::cout << "OptimizeVar EnterSearch" << std::endl;
  found_initial_solution_ = false;
  if (maximize_) {
    best_ = kint64min;
  } else {
...
void OptimizeVar::BeginNextDecision(DecisionBuilder* const db) {
	//std::cout << "OptimizeVar BeginNextDecision" << std::endl;
...
void OptimizeVar::ApplyBound() {
  if (found_initial_solution_) {
    if (maximize_) {
	//	std::cout<<"Applybound-Maximize"<<std::endl;
      var_->SetMin(best_ + step_);
	 // std::cout<<"Success"<<std::endl;
    } else {
	//	std::cout<<"Applybound-Minimize"<<std::endl;
...
void OptimizeVar::RefuteDecision(Decision* const d) { 
	//std::cout << "OptimizeVar RefuteDecision, varMin="<<var_->Min()<<" varMax="<<var_->Max() << std::endl; 
	ApplyBound(); 
}
...
bool OptimizeVar::AcceptSolution() {
  ...
  if (!found_initial_solution_) {
	...
	int64 varMax = var_->Max();
	int64 varMin = var_->Min();
	//std::cout<<"OptimizeVar AcceptSolution , varMin="<<varMin<<" varMax="<<varMax<<std::endl;
...
bool OptimizeVar::AtSolution() {
  ...
  //std::cout<<"OptimizerVar: at solution, varMin="<< var_->Min() <<" varMax="<< var_->Max()<<std::endl;
  if (maximize_) {
