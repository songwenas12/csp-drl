...
#include <typeinfo>
#include <chrono>
#include "ortools/constraint_solver/search.cc"
#include "ortools/constraint_solver/table.cc"
#include "ortools/constraint_solver/drl_constrStruct.cc"

#include "ortools/constraint_solver/drl_interface.h"
#pragma comment(lib,"ortools/constraint_solver/drlDLL.lib")
...
namespace operations_research {
	...
	std::default_random_engine Solver::generator;
	std::uniform_real_distribution<double> Solver::distribution(0.0, 1.0);
	...
	public:
		...
		//<ADD for DRL>
		std::shared_ptr<ConstrFeatures> origFeature;	//Describe the original instance 
...
#define CP_DO_FAIL(search) throw FailException()
	...
	Solver::~Solver() {
		...
		//Delete the added content
		this->varList.clear();
		this->constrList.clear();
		this->constrChecker.clear();
		//std::cout << "Destructing Solver " << this->name_ << std::endl;
	}
	...
	void Solver::PushState(Solver::MarkerType t, const StateInfo& info) {
		...
		m->origFeature = info.origFeature;	//<ADD for DRL>
	...
	Solver::MarkerType Solver::PopState(StateInfo* info) {
		...
		//Search* const tmpSearch = searches_.back();
		//for (auto iter = tmpSearch->monitors_.cbegin(); iter != tmpSearch->monitors_.cend(); iter++) {
		   // if (OptimizeVar* tmpVar = dynamic_cast<OptimizeVar*>(*iter)) {
			  //  //TODO: check the value of the optimizeVar  
			  //  std::cout << "OptimizerVar: Before PopState, varMin=" << tmpVar->Var()->Min()
			  //	  << " varMax=" << tmpVar->Var()->Max() << std::endl;
		   // }
		//}
		...
		(*info) = m->info_;
		info->origFeature = m->origFeature; // <ADD for DRL>
	...
	Constraint* Solver::AddConstraint_Return(Constraint* const c) {
	...
	void Solver::ProcessConstraints() {
		...
		// a FIFO way.
		//std::cout<<"Executing Solver::ProcessConstraints"<<std::endl;
	...
	bool Solver::BacktrackOneLevel(Decision** const fail_decision) {
		...
		while (!end_loop) {
			StateInfo info;
			Solver::MarkerType t = PopState(&info);	//For DRL: record point 
			if (this->drl_phase != DRLPhase::TESTING) {
				this->crtFeature = info.origFeature;	//For DRL: restore the instance state
			}
			switch (t) {
			case SENTINEL:
	...
	bool Solver::NextSolution_DRL() {
		...
		if (this->drl_phase != DRLPhase::TESTING) {
			this->crtFeature = std::make_shared<ConstrFeatures>(this, false);
		}
		volatile bool finish = false;
		volatile bool result = false;
		DecisionBuilder* const db = search->decision_builder();
		while (!finish) {
			CP_TRY(search) {
				if (fd != nullptr) {
					StateInfo i1(fd, 1, search->search_depth(),
						search->left_search_depth());  // 1 for right branch
					if (this->drl_phase != DRLPhase::TESTING) 
						i1.origFeature = this->crtFeature;
										
					PushState(CHOICE_POINT, i1);
					this->t0_cp = std::chrono::high_resolution_clock::now();
					// <RECORD POINT - Before> Only record if not in testing
					if (this->drl_phase != DRLPhase::TESTING) {
						this->recordBranchingVarID(fd);
					...
					auto t1_cp = std::chrono::high_resolution_clock::now();
					auto dur_cp = std::chrono::duration_cast<std::chrono::microseconds>(t1_cp - t0_cp).count();
					this->cpTime += dur_cp;

					// <RECORD POINT - After> Only record if not in testing
					if (this->drl_phase != DRLPhase::TESTING) {
						this->t1 = std::chrono::high_resolution_clock::now();
						auto newFeature = std::make_shared<ConstrFeatures>(this, /*this->masterDRLObj->numConstrCheckSample,*/ false);
						masterDRLObj->addTransactionToMemory(this->instanceConstrStruct,
							this->crtFeature, this->varOnDecision, 1, newFeature, false);
						this->crtFeature = newFeature;
					}
					if (this->drl_phase == DRLPhase::TRAINING) {
						this->totalTrainingLoss += masterDRLObj->Fit(masterDRLObj->crtLearningRate);
					}
				}
				Decision* d = nullptr;
				for (;;) {
					...
					if (d != nullptr) {
						...
						switch (modification) {
						case SWITCH_BRANCHES: {
							std::cout << "Error: in SWITCH_BRANCHES, not implemented" << std::endl;
							d = RevAlloc(new ReverseDecision(d));
							// We reverse the decision and fall through the normal code.
							FALLTHROUGH_INTENDED;
						}
						case NO_CHANGE: {
							...
							StateInfo i2(d, 0, search->search_depth(),
								search->left_search_depth());  // 0 for left branch
							if (this->drl_phase != DRLPhase::TESTING)
								i2.origFeature = this->crtFeature;
							
							PushState(CHOICE_POINT, i2);
							// <RECORD POINT - Before>
							if (this->drl_phase != DRLPhase::TESTING) {
								this->recordBranchingVarID(d);
								this->t0 = std::chrono::high_resolution_clock::now();
							}
							search->ApplyDecision(d);
							branches_++;
							...
							if (this->drl_phase != DRLPhase::TESTING) {
								...
								auto newFeature = std::make_shared<ConstrFeatures>(this, /*this->masterDRLObj->numConstrCheckSample,*/ false);
								masterDRLObj->addTransactionToMemory(this->instanceConstrStruct,
									this->crtFeature, this->varOnDecision, 1, newFeature, true);
								this->crtFeature = newFeature;
							}
							if (this->drl_phase == DRLPhase::TRAINING) {
								this->totalTrainingLoss += masterDRLObj->Fit(masterDRLObj->crtLearningRate);
						...
						case KEEP_LEFT: {
							std::cout << "Error: in KEEP_LEFT, not implemented" << std::endl;
							// <RECORD POINT - Before>
							search->ApplyDecision(d);
							d->Apply(this);
							// <RECORD POINT - After>
						...
						case KEEP_RIGHT: {
							std::cout << "Error: in KEEP_RIGHT, not implemented" << std::endl;
							// <RECORD POINT - Before>
							search->RefuteDecision(d);
							d->Refute(this);
							// <RECORD POINT - After>
						...
						case KILL_BOTH: {
							std::cout << "Error: in KILL_BOTH, not implemented" << std::endl;
							Fail();
						}
						}
					}
			...
			CP_ON_FAIL{
				...
				if (this->drl_phase != DRLPhase::TESTING) {
					this->t1 = std::chrono::high_resolution_clock::now();
					auto newFeature = std::make_shared<ConstrFeatures>(this, /*this->masterDRLObj->numConstrCheckSample,*/ true);
					masterDRLObj->addTransactionToMemory(this->instanceConstrStruct,
						this->crtFeature, this->varOnDecision, 1, newFeature, false);
				}
				if (this->drl_phase == DRLPhase::TRAINING) {
		...
		//Print the masterDRL information
		auto t1_real = std::chrono::high_resolution_clock::now();
		auto real_period = t1_real - t0_real;
		this->realTime = std::chrono::duration_cast<std::chrono::milliseconds>(real_period).count();
		if (this->drl_phase == DRLPhase::WARM_UP) {
			std::cout << "memCount=" << this->masterDRLObj->memoryCount << std::endl;
		}
		if (this->drl_phase == DRLPhase::TRAINING) {
			std::cout << "numIter=" << this->masterDRLObj->numIterations << ",  memCount=" << this->masterDRLObj->memoryCount
				<< ", totalTrainingLoss=" << std::setprecision(3) << this->totalTrainingLoss
				<< ", crtLR=" << this->masterDRLObj->crtLearningRate
				<< ", crtEPS=" << this->masterDRLObj->crtEps << std::endl;
			std::cout << "TotalInferenceTime=" << this->inferenceTime << ", inferenceCount=" << this->inferenceCount
				<< ",  TotalFeatureGenTime=" << this->featureGenTime << ", featureGenCount=" << this->featureCount << std::endl;
		}
		if (this->drl_phase == DRLPhase::TESTING) {
			std::cout << "CPTime = " << this->cpTime
				<< " InfrTime = " << inferenceTime
				<< std::endl;
		}
		return result;
	}

	bool Solver::Test(const std::vector<std::vector<int> > &arg) {
	...
	IMasterDRL* Solver::createMasterDRL(char* filePath) {
		std::cout << "Creating MasterDRL..." << std::endl;
		IMasterDRL* masterDRL = createMasterDRLFromDLL(filePath);
		return masterDRL;
	}

	void Solver::deleteMasterDRL(IMasterDRL* masterDRLObj) {
		masterDRLObj->destroy();
	}

	void Solver::setMasterDRL(IMasterDRL* masterDRLObj) {
		this->masterDRLObj = masterDRLObj;
		masterDRLObj->setSolver(this);

		//Compute the initial tightness
		//<TODO>: find a more suitable place for this
		this->initConstrTightness.resize(constrList.size());
	}

	void Solver::takeSnapshot(IMasterDRL* masterDRLObj) {
		masterDRLObj->TakeSnapshot();
	}

	void Solver::saveDRLModel(IMasterDRL* masterDRLObj, std::string modelFilePath) {
		masterDRLObj->SaveModel(modelFilePath);
	}

	void Solver::loadDRLModel(IMasterDRL* masterDRLObj, std::string modelFilePath) {
		masterDRLObj->LoadModel(modelFilePath);
	}
	...
	bool Solver::CheckAssignment(Assignment* const solution) {
		...
		CP_ON_FAIL{
		  ...
		  if (ct->name().empty()) {
			 ...
			 //LOG(INFO) << "Failing constraint = " << ct->name() << ":" << ct->DebugString();
		   }
		   queue_->AfterFailure();
		   BacktrackToSentinel(INITIAL_SEARCH_SENTINEL);
		   state_ = PROBLEM_INFEASIBLE;
	...
	void Solver::Fail() {
		//std::cout << "At Solver::Fail()" << std::endl;
		if (fail_intercept_) {
			fail_intercept_();
			return;
		}
	...
	void Solver::setVarConstrList(std::vector<IntVar*> varList, std::vector<Constraint*> constrList) {
		this->varList = varList;
		this->constrList = constrList;
	}

	std::shared_ptr<ConstrStruct> Solver::CreateConstrStruct(int caseID, std::vector<std::vector<int>> varConstrIndex,
		std::vector<std::vector<int>> constrVarIndex) {
		return std::make_shared<ConstrStruct>(caseID, varConstrIndex, constrVarIndex);
	}

	void Solver::setConstrStruct(std::shared_ptr<ConstrStruct> instanceStruct) {
		this->instanceConstrStruct = instanceStruct;
	}

	bool Solver::checkIfCaseInPool(IMasterDRL* masterDRLObj, int caseID) {
		return masterDRLObj->checkIfCaseInPool(caseID);
	}

	std::shared_ptr<ConstrStruct> Solver::retrieveConstrStruct(IMasterDRL* masterDRLObj, int caseID) {
		return masterDRLObj->retrieveConstrStruct(caseID);
	}

	void Solver::addConstrStructToPool(IMasterDRL* masterDRLObj, int caseID, std::shared_ptr<ConstrStruct> constrStruct) {
		masterDRLObj->addConstrStructToPool(caseID, constrStruct);
	}

	void Solver::addConstrChecker(Solver* checker) {
		this->constrChecker.push_back(checker);
		/*std::cout << "Checker added." << std::endl;
		std::cout << this->constrChecker.back() << std::endl;*/
	}

	void Solver::addCheckerAssignment(Assignment* assignmnet) {
		this->checkerAssignment.push_back(assignmnet);
	}

	void Solver::addCheckerVarMapping(std::vector<int> mapping) {
		this->checkerVarMapping.push_back(mapping);
	}

	//Compute the state transition cost (in microseconds)
	double Solver::getTransitionCost() {
		auto time = this->t1 - this->t0;
		auto count = std::chrono::duration_cast<std::chrono::microseconds>(time).count();
		//std::cout << "Cost " << count << " double value=" << (double)count << std::endl;
		return (double)count/this->varList.size();	//Normalize the cost
		//return count;
	}

	void Solver::recordBranchingVarID(Decision* decision) {
		this->varOnDecision= -1;
		AssignOneVariableValue* tmp_d = dynamic_cast<AssignOneVariableValue*>(decision);
		if (tmp_d != nullptr) {
			this->varOnDecision = tmp_d->getVarID(this->varList);
			//std::cout << "Chosen VarID=" << this->varOnDecision << std::endl;
		}
		else
			std::cout << "Error: Decision type is not AssignOneVariableValue!" << std::endl;
	}

	bool Solver::GreedyOrNot(double eps) {
		double randomValue = this->distribution(this->generator);
		//std::cout << "GreedyOrNot, randomValue="<<randomValue<<", eps=" << eps << std::endl;
		if (randomValue >= eps)
			return true;
		return false;
	}

	void Solver::setSolverPhase(DRLPhase phase) {
		this->drl_phase = phase;
	}
