...
namespace operations_research {
	...
	public:
		...
		enum IntVarStrategy {
			...
			// <ADD for DRL> Among unbound variables, select the variable with the minimum Q value.
			CHOOSE_MIN_Q,

			CHOOSE_MIN_Q_TEST,

			CHOOSE_MIN_SIZE_OVER_DDEG,

			CHOOSE_MIN_SIZE_OVER_TDEG
		};
		...
		Constraint* AddConstraint_Return(Constraint* const c);
		...
		IMasterDRL* masterDRLObj;
		DRLPhase drl_phase;
		std::vector<IntVar*> varList;
		std::vector<Constraint*> constrList;
		std::shared_ptr<ConstrStruct> instanceConstrStruct;
		std::vector<Solver*> constrChecker;	//The elements will be auto destructed by python
		std::vector<Assignment*> checkerAssignment;	//The elements will be auto destructed by python
		std::vector<std::vector<int>> checkerVarMapping;
		std::vector<double> initConstrTightness;	//The initial tigthness
		std::chrono::time_point<std::chrono::high_resolution_clock> t0, t1;
		std::chrono::time_point<std::chrono::high_resolution_clock> t0_cp;
		int64 realTime = 0;	//Real time spent in solving (in milliseconds)
		std::shared_ptr<ConstrFeatures> crtFeature;	//Describe the current state of the instance
		double transCost = 0.0;
		int varOnDecision = -1;
		double featureGenTime = 0.0;	//In milliseconds, only record the inference time for TESTING phase
		int featureCount = 0;	//Count the feature generation times, only for the TESTING phase
		int64 inferenceTime = 0;		//In milliseconds, only record the inference time for TESTING phase
		int inferenceCount = 0; 		//Count the inference times, only for the TESTING phase
		int64 cpTime = 0;	//The time used for constraint propagation
		//int64 cp_on_fail_time = 0;	//The time spent in CP_ON_FAIL
		double avgInferTime = 0.0;	//The running estimate of inference time
		double totalTrainingLoss = 0.0;
		static std::default_random_engine generator;
		static std::uniform_real_distribution<double> distribution;
		
		void printVars();
		void setVarConstrList(std::vector<IntVar*> varList, std::vector<Constraint*> constrList);
		std::shared_ptr<ConstrStruct> CreateConstrStruct(int caseID, std::vector<std::vector<int>> varConstrIndex,
			std::vector<std::vector<int>> constrVarIndex);
		bool NextSolution_DRL();
		int64 getNumDecisions() { return this->decisions_; }
		int64 getRealSolvingTime() { return this->realTime;  }
		int64 getInferenceTime() { return this->inferenceTime; }
		bool Test(const std::vector<std::vector<int>> &arg);
		double getTransitionCost();
		void recordBranchingVarID(Decision* decision);
		bool GreedyOrNot(double eps);

		static IMasterDRL* createMasterDRL(char* filePath);
		static void deleteMasterDRL(IMasterDRL* masterDRLObj);
		void setMasterDRL(IMasterDRL* masterDRLObj);
		void setConstrStruct(std::shared_ptr<ConstrStruct> instanceStruct);
		bool checkIfCaseInPool(IMasterDRL* masterDRLObj, int caseID);
		std::shared_ptr<ConstrStruct> retrieveConstrStruct(IMasterDRL* masterDRLObj, int caseID);
		void addConstrStructToPool(IMasterDRL* masterDRLObj, int caseID, std::shared_ptr<ConstrStruct> constrStruct);
		void addConstrChecker(Solver* checker);
		void addCheckerAssignment(Assignment* assignment);
		void addCheckerVarMapping(std::vector<int> checkerVarMapping);
		void setSolverPhase(DRLPhase phase);
		static void takeSnapshot(IMasterDRL* masterDRLObj);
		static void saveDRLModel(IMasterDRL* masterDRLObj, std::string modelFilePath);
		static void loadDRLModel(IMasterDRL* masterDRLObj, std::string modelFilePath);
	...
	//This class records the structure of a constraint satsifaction instance, 
	//i.e. the hypergraph structure of the constraint network.
	//Detailed info (e.g. domains, edge constrainedness) is recorded using the 
	//ConstrFeatures class.
	class ConstrStruct {

	public:
		int caseID;	//The id of the instance that this struct represents 
		int numVars;
		int numConstrs;

		std::vector<std::vector<int>> varConstrIndex;	//For each variable, record the 
														//id of the involved constraints
		std::vector<std::vector<int>> constrVarIndex;	//For each constraint, record the 
														//id of the involved variables
		std::vector<std::set<int>> varNeighborIndex; //For each variable, record the id 
													 //of its neighbor, i.e. connect by a constraint
		//Methods
		ConstrStruct(int caseID, std::vector<std::vector<int>> varConstrIndex, std::vector<std::vector<int>> constrVarIndex);
		~ConstrStruct();

		void printVarSize(std::vector<int> &varSizeList);

	};

	//This class holds the information that describes the details of a ConstrStruct
	class ConstrFeatures {

	public:
		std::shared_ptr<ConstrStruct> relateConstrStruct;
		bool ifTerminal;	//If this state is a terminal one (either feasible or a solution)
		
		//For vars
		std::vector<int> varSize;	//The domain size of the var
		std::vector<int> varMin;	//The min value of each var 
		std::vector<int> varMax;	//The max value of each var
		std::vector<int> ifBound;	//If the variable is bounded (0 for not, 1 for is)
		std::vector<int> ddeg;		//The dynamic degree of the var

		//For constraints
		std::vector<double> tightness;	//The tightness of each constraint
		std::vector<int> num_unbounded;	//The number of unbounded variables associated with each constraint
		std::vector<int> domainProduct;	//The number of all possible tuples
		
		//Methods
		ConstrFeatures(Solver* relatedSolver, /*int sampleNum,*/ bool ifFail);
		~ConstrFeatures();
		
		//For table constraints, compute the tightness
		double computeTableConstrTightness(Solver* relatedSolver, int constrID);
	...
	public:
		...
		//<ADD for DRL> Get the current number of active tuples
		virtual uint64 numActiveTuples() {
			std::cout << "Error: Printing from Constraint Class" << std::endl;
			return 0;
		}
